...
proc is_skip_net {name} {
    if {$name == "clk" || $name == "rst_n" || $name == "clk_i" || $name == "rst_ni" || $name =="CDN" || $name == "CP"} {
        return 1
    }
...
#create shares for each port bus in the top module (as long as it's not clk,rst,randbits)
proc create_shared_top_ports {top} {
    puts "create_shared_top_ports was called"
    set port_busses [get_db port_busses]
    #go through all port_busses in top module
...
#same as above, but for every other module
...
proc replace_cells_with_secure {suffix} {
    ...
    set pmaps("base_cell:INVX1")     {{A I_S0} {Y I_S0}}
...
proc connect_shared_wires {top suffix} {
	...
	#only used to check if we skip signal
        ...
        if {[is_skip_net [get_db $hnet_load .base_name]] == 1} {
            ...
            } elseif {[get_object_type $hnet_load] == "hpin"} {
                set hnet_load_hinst     [get_db $hnet_load .hinst.module.base_name]
            } elseif {[get_object_type $hnet_load] == "pin"} {
		        set hnet_load_hinst     [get_db $hnet_load .inst.lib_cells.base_name]
            } else {
		        set hnet_load_hinst     [get_db $hnet_load .hinst.module.base_name]
	    }
	  puts "checking for dead cell"  
	  if {[get_db $hnet .drivers] == ""} {
		  continue
	   }		
	   puts "cell is ALIVE"
            ...
            } elseif { [get_object_type $hnet_driver] == "pin" } {
                set hnet_driver_hinst     [get_db $hnet_driver .inst.lib_cells.base_name]
                set hnet_driver_type      [get_db $hnet_driver .obj_type]
            } else {
                set hnet_driver_hinst     [get_db $hnet_driver .hinst.module.base_name]
                set hnet_driver_type      [get_db $hnet_driver .obj_type]
            }
		#sometimes driver will be undefined if a parameter is set to 0 --> net wont be driven to
	   ...
	   # if { [string length $hnet_driver_hinst] == 0 } {
 	#	   continue
	#	}
	    ...
	    eval [list connect -net_name $hnet_name_share $hnet_driver_share $hnet_load_share]
