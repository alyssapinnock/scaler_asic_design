...
proc connect_shared_wires {top suffix} {
    ...
    foreach hnet $hnets {
        ...
        if { [is_secure_gate $hnet_name_hinst $suffix] == 1 } {
            puts "    Skipped (base cell) hnet: $hnet_name"
            continue
        }
        ...
        #stores the name of the module the driver belongs to and the type of object the driver is (port, pin, hpin etc)
        ...
        if { $hnet_driver_obj_type == "port" } {
            ...
            # For pins you must find the instance it belongs to and then find the module of that instance to get the hierarchical name
        ...
        #Skip constant-driven clk/rst/cdn/cp loads (optimized-out modules)
        if { $hnet_driver_type == "constant" } {
            ...
            foreach hnet_load $hnet_loads {
                ...
                lappend filtered_loads $hnet_load
            }
            set hnet_loads $filtered_loads
            if { [llength $hnet_loads] == 0 } {
                puts "    Skipped (const->clk/rst/cdn/cp only) hnet: $hnet_name"
                continue
            }
        }

        #Determine the shared net name for the driver. This is needed later to connect to the loads
        ...
        foreach hnet_load $hnet_loads {
            ...
            if { $hnet_load_obj_type == "port" || $hnet_load_obj_type == "hport" } {
                puts "    Load is a port: $hnet_load. Going to connect first"
                set port_load $hnet_load
                break
            }
        }

        if { $port_load != "" && [is_skip_net [get_db $port_load .base_name]]==0} {
            set hnet_load_hinst     "$top"
            set hnet_load_share     [append_s1 $port_load]
            #Use the shared port name for -net_name to avoid name mismatches
        ...
        foreach hnet_load $hnet_loads {
            if { $port_load != "" && $hnet_load == $port_load || [is_skip_net [get_db $port_load .base_name]]==1} {
		    continue
            }
            ...
            puts "        Connecting: $connect"
            if { [is_skip_net [get_db $hnet_load .base_name]]==1} {
	    	puts "should skip hnet"
		continue
		}
	    eval $connect
        }
...
proc connect_random_bus {nregister suffix} {
    ...
    if { $nregister <= 0 } {
        puts "Invalid randbits width: $nregister"
        suspend
    }
    ...
    foreach hinst [get_db hinsts] {
        ...
        if { [string match "DFCNQ_${suffix}*" $module_name] == 1 } {
            set bus_idx [expr {$i % $width}]
            set busat   "randbits\[$bus_idx\]"
